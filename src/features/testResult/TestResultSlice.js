...
export const updatePatientTestResults = createAsyncThunk(
  ...
  async ({ patientTestId, testId, updateData }, { rejectWithValue }) => {
    try {
      ...
      console.log("🧪 Updating test results:", patientTestId, testId, updateData);
      const response = await axios.patch(
        `${API_URL}/testResult/${patientTestId}/tests/${testId}/results`,
        updateData,
        { headers: getAuthHeaders() }
      );

      console.log("✅ Test results updated:", response?.data?.data);
      return response.data.data;
    } catch (error) {
      const message = error.response?.data?.message || error.message || 'Failed to update test results';
