...
const CreateRadiologyReport = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editId) {
                await axios.put(
                    `http://localhost:5000/radiology/update-report/${editId}`,
                    {
                        ...form,
                        finalContent,
                    }
                ...
                await axios.post("http://localhost:5000/radiology/create-report", {
                    ...form,
                    finalContent,
    ...
    const fetchReports = async () => {
        try {
            const res = await axios.get("http://localhost:5000/radiology/get-report");
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this report?")) {
            try {
                await axios.delete(`http://localhost:5000/radiology/delete-report/${id}`);
                ...
                <div className="flex space-x-3">
                    ...
                    >
                        {loading ? (
                            ...
                            editId ? "Update Report" : "Generate & Save Report"
                        )}
                    </button>
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-600">MRNO: {report.patientMRNO}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-sm">{new Date(report.date).toLocaleDateString()}</p>
                                        <p className="text-sm text-gray-600">{report.age} yrs, {report.sex}</p>
                                    </div>
                                </div>
                                ...
                                <details className="mt-3 group">
                                    <summary className="flex items-center cursor-pointer text-blue-600 hover:text-blue-800">
                                        ...
                                        <svg className="ml-1 w-4 h-4 group-open:rotate-180 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
