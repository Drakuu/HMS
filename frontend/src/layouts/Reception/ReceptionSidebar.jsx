...
const ReceptionSidebar = () => {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      links: [
        { href: "/ward-management", label: "ward Management" },
      ]
    },
    {
      name: "Pharmacy",
    ...
    },
    {
      ...
      links: [
        { href: "/account/bill-list", label: "Bill list" },
      ],
    },
    {
      name: "Inventory",
  ...
  return (
    <div className="w-64 h-screen bg-primary-500 text-white flex flex-col shadow-lg">
          ...
          <div key={index} className="mb-1">
            ...
            <div
              className={`flex items-center justify-between px-4 py-3 cursor-pointer transition-colors ${
                activeMenu === menu.name ? 'bg-primary-600' : 'hover:bg-primary-600'
              }`}
                ...
                {menu.links.map((link, linkIndex) => (
                  <Link
                    ...
                    className={`block px-3 py-2 text-sm transition-colors rounded ${
                      activeSubMenu === link.label ? 'bg-primary-600 text-white' : 'hover:bg-primary-600 text-white'
                    }`}
                  >
                    {link.label}
                  </Link>
      ...
      <div className="p-4 border-t">
        <button
          className="flex items-center justify-center w-full px-4 py-2 rounded-lg text-white font-medium transition-colors"
        >
          That's it 
        </button>
      </div>
